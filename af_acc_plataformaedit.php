...
$af_acc_plataforma_edit = NULL; // Initialize page object first

class caf_acc_plataforma_edit extends caf_acc_plataforma {
	...
	// Page object name
	var $PageObjName = 'af_acc_plataforma_edit';
	...
	function Page_Main() {
		...
		if ($this->cl_Accion->CurrentValue == "")
			$this->Page_Terminate("af_acc_plataformalist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("af_acc_plataformalist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Save old values
			$rsold = &$rs->fields;
			$this->LoadDbValues($rsold); //echo "<pre>"; print_r($rsold); echo "</pre>";
			$rsnew = array();
...
// Create page object
if (!isset($af_acc_plataforma_edit)) $af_acc_plataforma_edit = new caf_acc_plataforma_edit();

// Page init
$af_acc_plataforma_edit->Page_Init();

// Page main
$af_acc_plataforma_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_acc_plataforma_edit->Page_Render();
?>
...
// Page object
var af_acc_plataforma_edit = new ew_Page("af_acc_plataforma_edit");
af_acc_plataforma_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = af_acc_plataforma_edit.PageID; // For backward compatibility

// Form object
var faf_acc_plataformaedit = new ew_Form("faf_acc_plataformaedit");

// Validate form
faf_acc_plataformaedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_plataforma->cl_Accion->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_t_Accion");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_plataforma->t_Accion->FldCaption()) ?>");
...
faf_acc_plataformaedit.Form_CustomValidate = 
...
faf_acc_plataformaedit.ValidateRequired = true;
<?php } else { ?>
faf_acc_plataformaedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_acc_plataformaedit.Lists["x_cl_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_plataformaedit.Lists["x_t_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $af_acc_plataforma_edit->ShowPageHeader(); ?>
<?php
$af_acc_plataforma_edit->ShowMessage();
?>
<form name="faf_acc_plataformaedit" id="faf_acc_plataformaedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_acc_plataforma">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="k_hash" id="k_hash" value="<?php echo $af_acc_plataforma_edit->HashValue ?>">
<div id="page_title"> - Editar</div>
<table class="ewGrid"><tr><td>
<table id="tbl_af_acc_plataformaedit" class="table table-bordered table-striped">
...
<?php echo $af_acc_plataforma->cl_Accion->EditValue ?></span>
</span>
<input type="hidden" data-field="x_cl_Accion" name="x_cl_Accion" id="x_cl_Accion" value="<?php echo ew_HtmlEncode($af_acc_plataforma->cl_Accion->CurrentValue) ?>">
<?php echo $af_acc_plataforma->cl_Accion->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php echo $af_acc_plataforma->t_Accion->EditValue ?></span>
</span>
<input type="hidden" data-field="x_t_Accion" name="x_t_Accion" id="x_t_Accion" value="<?php echo ew_HtmlEncode($af_acc_plataforma->t_Accion->CurrentValue) ?>">
<?php echo $af_acc_plataforma->t_Accion->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<input type="text" data-field="x_x_DirCorreo" name="x_x_DirCorreo" id="x_x_DirCorreo" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($af_acc_plataforma->x_DirCorreo->PlaceHolder) ?>" value="<?php echo $af_acc_plataforma->x_DirCorreo->EditValue ?>"<?php echo $af_acc_plataforma->x_DirCorreo->EditAttributes() ?>>
</span>
...
<?php if ($af_acc_plataforma->x_Titulo->Visible) { // x_Titulo ?>
	<tr id="r_x_Titulo">
		<td>
			...
			<span id="elh_af_acc_plataforma_x_Titulo"><?php echo $af_acc_plataforma->x_Titulo->FldCaption() ?></span></td>
...
<?php if ($af_acc_plataforma->x_Mensaje->Visible) { // x_Mensaje ?>
	<tr id="r_x_Mensaje">
		<td>
			...
			<span id="elh_af_acc_plataforma_x_Mensaje"><?php echo $af_acc_plataforma->x_Mensaje->FldCaption() ?></span></td>
...
<?php if ($af_acc_plataforma->UpdateConflict == "U") { // Record already updated by other user ?>
...
<script type="text/javascript">
faf_acc_plataformaedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$af_acc_plataforma_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_acc_plataforma_edit->Page_Terminate();
?>
