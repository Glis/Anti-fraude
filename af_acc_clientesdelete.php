...
$af_acc_clientes_delete = NULL; // Initialize page object first

class caf_acc_clientes_delete extends caf_acc_clientes {
	...
	// Page object name
	var $PageObjName = 'af_acc_clientes_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("af_acc_clienteslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in af_acc_clientes class, af_acc_clientesinfo.php
...
// Create page object
if (!isset($af_acc_clientes_delete)) $af_acc_clientes_delete = new caf_acc_clientes_delete();

// Page init
$af_acc_clientes_delete->Page_Init();

// Page main
$af_acc_clientes_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_acc_clientes_delete->Page_Render();
?>
...
// Page object
var af_acc_clientes_delete = new ew_Page("af_acc_clientes_delete");
af_acc_clientes_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = af_acc_clientes_delete.PageID; // For backward compatibility

// Form object
var faf_acc_clientesdelete = new ew_Form("faf_acc_clientesdelete");

// Form_CustomValidate event
faf_acc_clientesdelete.Form_CustomValidate = 
...
faf_acc_clientesdelete.ValidateRequired = true;
<?php } else { ?>
faf_acc_clientesdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_acc_clientesdelete.Lists["x_cl_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_clientesdelete.Lists["x_t_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_clientesdelete.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_clientesdelete.Lists["x_c_ICClass"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($af_acc_clientes_delete->Recordset = $af_acc_clientes_delete->LoadRecordset())
	$af_acc_clientes_deleteTotalRecs = $af_acc_clientes_delete->Recordset->RecordCount(); // Get record count
if ($af_acc_clientes_deleteTotalRecs <= 0) { // No record found, exit
	if ($af_acc_clientes_delete->Recordset)
		$af_acc_clientes_delete->Recordset->Close();
	$af_acc_clientes_delete->Page_Terminate("af_acc_clienteslist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $af_acc_clientes_delete->ShowPageHeader(); ?>
<?php
$af_acc_clientes_delete->ShowMessage();
?>
<form name="faf_acc_clientesdelete" id="faf_acc_clientesdelete" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_acc_clientes">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($af_acc_clientes_delete->RecKeys as $key) { ?>
...
<table id="tbl_af_acc_clientesdelete" class="ewTable ewTableSeparate">
<?php echo $af_acc_clientes->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($af_acc_clientes->cl_Accion->Visible) { // cl_Accion ?>
		<td><span id="elh_af_acc_clientes_cl_Accion" class="af_acc_clientes_cl_Accion"><?php echo $af_acc_clientes->cl_Accion->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_clientes->t_Accion->Visible) { // t_Accion ?>
		<td><span id="elh_af_acc_clientes_t_Accion" class="af_acc_clientes_t_Accion"><?php echo $af_acc_clientes->t_Accion->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_clientes->c_IReseller->Visible) { // c_IReseller ?>
		<td><span id="elh_af_acc_clientes_c_IReseller" class="af_acc_clientes_c_IReseller"><?php echo $af_acc_clientes->c_IReseller->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_clientes->c_ICClass->Visible) { // c_ICClass ?>
		<td><span id="elh_af_acc_clientes_c_ICClass" class="af_acc_clientes_c_ICClass"><?php echo $af_acc_clientes->c_ICClass->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$af_acc_clientes_delete->RecCnt = 0;
$i = 0;
while (!$af_acc_clientes_delete->Recordset->EOF) {
	$af_acc_clientes_delete->RecCnt++;
	$af_acc_clientes_delete->RowCnt++;

	// Set row properties
	$af_acc_clientes->ResetAttrs();
	$af_acc_clientes->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$af_acc_clientes_delete->LoadRowValues($af_acc_clientes_delete->Recordset);

	// Render row
	$af_acc_clientes_delete->RenderRow();
?>
	<tr<?php echo $af_acc_clientes->RowAttributes() ?>>
<?php if ($af_acc_clientes->cl_Accion->Visible) { // cl_Accion ?>
		<td<?php echo $af_acc_clientes->cl_Accion->CellAttributes() ?>>
<span id="el<?php echo $af_acc_clientes_delete->RowCnt ?>_af_acc_clientes_cl_Accion" class="control-group af_acc_clientes_cl_Accion">
...
<span id="el<?php echo $af_acc_clientes_delete->RowCnt ?>_af_acc_clientes_t_Accion" class="control-group af_acc_clientes_t_Accion">
...
<span id="el<?php echo $af_acc_clientes_delete->RowCnt ?>_af_acc_clientes_c_IReseller" class="control-group af_acc_clientes_c_IReseller">
...
<span id="el<?php echo $af_acc_clientes_delete->RowCnt ?>_af_acc_clientes_c_ICClass" class="control-group af_acc_clientes_c_ICClass">
...
<?php
	$af_acc_clientes_delete->Recordset->MoveNext();
}
$af_acc_clientes_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
faf_acc_clientesdelete.Init();
</script>
<?php
$af_acc_clientes_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_acc_clientes_delete->Page_Terminate();
?>
