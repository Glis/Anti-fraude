...
$af_acc_cuentas_edit = NULL; // Initialize page object first

class caf_acc_cuentas_edit extends caf_acc_cuentas {
	...
	// Page object name
	var $PageObjName = 'af_acc_cuentas_edit';
	...
	function Page_Main() {
		...
		if ($this->cl_Accion->CurrentValue == "")
			$this->Page_Terminate("af_acc_cuentaslist.php"); // Invalid key, return to list
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					...
					$rswrk->Close();
					$result = select_sql_PO("select_porta_customers_where", array($this->c_IReseller->CurrentValue));
					$this->c_IReseller->EditValue = $result[1]['name'];print_r($result);
				} else {
	...
	function GetRowHash(&$rs) {
		...
		$sHash .= ew_GetFldHash($rs->fields('f_Ult_Mod')); // f_Ult_Mod
		$sHash .= ew_GetFldHash($rs->fields('c_Usuario_Ult_Mod')); // c_Usuario_Ult_Mod
		return md5($sHash);
	}
...
// Create page object
if (!isset($af_acc_cuentas_edit)) $af_acc_cuentas_edit = new caf_acc_cuentas_edit();

// Page init
$af_acc_cuentas_edit->Page_Init();

// Page main
$af_acc_cuentas_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_acc_cuentas_edit->Page_Render();
?>
...
// Page object
var af_acc_cuentas_edit = new ew_Page("af_acc_cuentas_edit");
af_acc_cuentas_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = af_acc_cuentas_edit.PageID; // For backward compatibility

// Form object
var faf_acc_cuentasedit = new ew_Form("faf_acc_cuentasedit");

// Validate form
faf_acc_cuentasedit.Validate = function() {
...
faf_acc_cuentasedit.Form_CustomValidate = 
...
faf_acc_cuentasedit.ValidateRequired = true;
<?php } else { ?>
faf_acc_cuentasedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_acc_cuentasedit.Lists["x_cl_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasedit.Lists["x_t_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasedit.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasedit.Lists["x_c_ICClass"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $af_acc_cuentas_edit->ShowPageHeader(); ?>
<?php
$af_acc_cuentas_edit->ShowMessage();
?>
<form name="faf_acc_cuentasedit" id="faf_acc_cuentasedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_acc_cuentas">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="k_hash" id="k_hash" value="<?php echo $af_acc_cuentas_edit->HashValue ?>">
<div id="page_title"> - Editar</div>
<table class="ewGrid"><tr><td>
<table id="tbl_af_acc_cuentasedit" class="table table-bordered table-striped">
...
<?php echo $af_acc_cuentas->cl_Accion->EditValue ?></span>
</span>
<input type="hidden" data-field="x_cl_Accion" name="x_cl_Accion" id="x_cl_Accion" value="<?php echo ew_HtmlEncode($af_acc_cuentas->cl_Accion->CurrentValue) ?>">
...
<?php echo $af_acc_cuentas->t_Accion->EditValue ?></span>
</span>
<input type="hidden" data-field="x_t_Accion" name="x_t_Accion" id="x_t_Accion" value="<?php echo ew_HtmlEncode($af_acc_cuentas->t_Accion->CurrentValue) ?>">
...
<?php echo $af_acc_cuentas->c_IReseller->EditValue ?></span>
</span>
<input type="hidden" data-field="x_c_IReseller" name="x_c_IReseller" id="x_c_IReseller" value="<?php echo ew_HtmlEncode($af_acc_cuentas->c_IReseller->CurrentValue) ?>">
...
<?php echo $af_acc_cuentas->c_ICClass->EditValue ?></span>
</span>
<input type="hidden" data-field="x_c_ICClass" name="x_c_ICClass" id="x_c_ICClass" value="<?php echo ew_HtmlEncode($af_acc_cuentas->c_ICClass->CurrentValue) ?>">
...
<input class="form-control" type="email" data-field="x_x_DirCorreo" name="x_x_DirCorreo" id="x_x_DirCorreo" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($af_acc_cuentas->x_DirCorreo->PlaceHolder) ?>" value="<?php echo $af_acc_cuentas->x_DirCorreo->EditValue ?>"<?php echo $af_acc_cuentas->x_DirCorreo->EditAttributes() ?>>
</span>
...
<?php if ($af_acc_cuentas->UpdateConflict == "U") { // Record already updated by other user ?>
...
<script type="text/javascript">
faf_acc_cuentasedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$af_acc_cuentas_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_acc_cuentas_edit->Page_Terminate();
?>
