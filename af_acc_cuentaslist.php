...
class caf_acc_cuentas_list extends caf_acc_cuentas {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"btn-primary ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.faf_acc_cuentaslist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = (TRUE);
...
<script type="text/javascript">
	$(document).on('click','#submit_filtros',function(){
		...
		var dataString = "pag=acc_cuentas&filtro=clase_accion";
		if (clase_accion == ""){
		...
		$.ajax({  
		  ...
		  success: function(html) {  
			window.location="af_acc_cuentaslist.php?start=1";
		  }
		});
	});
	...
	$(document).on('click','#reset_fil',function(){
		...
		$.ajax({  
		  ...
		  data: dataString,  
		  success: function(html) { 
			window.location="af_acc_cuentaslist.php";
		  }
		});
	});
