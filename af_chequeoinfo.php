...
//
// Table class for af_chequeo
//
class caf_chequeo extends cTable {
	var $c_IChequeo;
	var $f_Inicio;
	var $f_Fin;
	var $f_Inicio_Vent;
	...
	function __construct() {
		...
		$this->TableVar = 'af_chequeo';
		$this->TableName = 'af_chequeo';
		...
		// c_IChequeo
		$this->c_IChequeo = new cField('af_chequeo', 'af_chequeo', 'x_c_IChequeo', 'c_IChequeo', '`c_IChequeo`', '`c_IChequeo`', 3, -1, FALSE, '`c_IChequeo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->c_IChequeo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['c_IChequeo'] = &$this->c_IChequeo;

		// f_Inicio
		$this->f_Inicio = new cField('af_chequeo', 'af_chequeo', 'x_f_Inicio', 'f_Inicio', '`f_Inicio`', '`f_Inicio`', 200, 9, FALSE, '`f_Inicio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['f_Inicio'] = &$this->f_Inicio;

		// f_Fin
		$this->f_Fin = new cField('af_chequeo', 'af_chequeo', 'x_f_Fin', 'f_Fin', '`f_Fin`', '`f_Fin`', 200, 9, FALSE, '`f_Fin`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['f_Fin'] = &$this->f_Fin;

		// f_Inicio_Vent
		$this->f_Inicio_Vent = new cField('af_chequeo', 'af_chequeo', 'x_f_Inicio_Vent', 'f_Inicio_Vent', '`f_Inicio_Vent`', '`f_Inicio_Vent`', 200, 9, FALSE, '`f_Inicio_Vent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['f_Inicio_Vent'] = &$this->f_Inicio_Vent;

		// f_Fin_Vent
		$this->f_Fin_Vent = new cField('af_chequeo', 'af_chequeo', 'x_f_Fin_Vent', 'f_Fin_Vent', '`f_Fin_Vent`', '`f_Fin_Vent`', 200, 9, FALSE, '`f_Fin_Vent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['f_Fin_Vent'] = &$this->f_Fin_Vent;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("af_chequeoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("af_chequeoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "af_chequeoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("af_chequeoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("af_chequeoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("af_chequeodelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["c_IChequeo"]; // c_IChequeo
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->c_IChequeo->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->f_Inicio_Vent->Exportable) $Doc->ExportCaption($this->f_Inicio_Vent);
				if ($this->f_Fin_Vent->Exportable) $Doc->ExportCaption($this->f_Fin_Vent);
			}
			$Doc->EndExportRow();
		}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->f_Inicio_Vent->Exportable) $Doc->ExportField($this->f_Inicio_Vent);
					if ($this->f_Fin_Vent->Exportable) $Doc->ExportField($this->f_Fin_Vent);
				}
				$Doc->EndExportRow();
			}
			$Recordset->MoveNext();
