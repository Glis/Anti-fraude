...
$af_acc_cuentas_add = NULL; // Initialize page object first

class caf_acc_cuentas_add extends caf_acc_cuentas {
	...
	// Page object name
	var $PageObjName = 'af_acc_cuentas_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("af_acc_cuentaslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "af_acc_cuentasview.php")
...
// Create page object
if (!isset($af_acc_cuentas_add)) $af_acc_cuentas_add = new caf_acc_cuentas_add();

// Page init
$af_acc_cuentas_add->Page_Init();

// Page main
$af_acc_cuentas_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_acc_cuentas_add->Page_Render();
?>
...
// Page object
var af_acc_cuentas_add = new ew_Page("af_acc_cuentas_add");
af_acc_cuentas_add.PageID = "add"; // Page ID
var EW_PAGE_ID = af_acc_cuentas_add.PageID; // For backward compatibility

// Form object
var faf_acc_cuentasadd = new ew_Form("faf_acc_cuentasadd");

// Validate form
faf_acc_cuentasadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_cuentas->cl_Accion->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_t_Accion");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_cuentas->t_Accion->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_c_IReseller");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_cuentas->c_IReseller->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_c_ICClass");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($af_acc_cuentas->c_ICClass->FldCaption()) ?>");
...
faf_acc_cuentasadd.Form_CustomValidate = 
...
faf_acc_cuentasadd.ValidateRequired = true;
<?php } else { ?>
faf_acc_cuentasadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_acc_cuentasadd.Lists["x_cl_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasadd.Lists["x_t_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasadd.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cuentasadd.Lists["x_c_ICClass"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $af_acc_cuentas_add->ShowPageHeader(); ?>
<?php
$af_acc_cuentas_add->ShowMessage();
?>
<form name="faf_acc_cuentasadd" id="faf_acc_cuentasadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_acc_cuentas">
<input type="hidden" name="a_add" id="a_add" value="A">
<div id="page_title"> - Añadir</div>
<table class="ewGrid"><tr><td>
<table id="tbl_af_acc_cuentasadd" class="table table-bordered table-striped">
<?php if ($af_acc_cuentas->cl_Accion->Visible) { // cl_Accion ?>
	<tr id="r_cl_Accion">
		<td><span id="elh_af_acc_cuentas_cl_Accion"><?php echo $af_acc_cuentas->cl_Accion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $af_acc_cuentas->cl_Accion->CellAttributes() ?>>
<span id="el_af_acc_cuentas_cl_Accion" class="control-group">
<select class="form-control" data-field="x_cl_Accion" id="x_cl_Accion" name="x_cl_Accion"<?php echo $af_acc_cuentas->cl_Accion->EditAttributes() ?>>
<?php
if (is_array($af_acc_cuentas->cl_Accion->EditValue)) {
	$arwrk = $af_acc_cuentas->cl_Accion->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($af_acc_cuentas->cl_Accion->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
faf_acc_cuentasadd.Lists["x_cl_Accion"].Options = <?php echo (is_array($af_acc_cuentas->cl_Accion->EditValue)) ? ew_ArrayToJson($af_acc_cuentas->cl_Accion->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $af_acc_cuentas->cl_Accion->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->t_Accion->Visible) { // t_Accion ?>
	<tr id="r_t_Accion">
		<td><span id="elh_af_acc_cuentas_t_Accion"><?php echo $af_acc_cuentas->t_Accion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $af_acc_cuentas->t_Accion->CellAttributes() ?>>
<span id="el_af_acc_cuentas_t_Accion" class="control-group">
<select class="form-control" data-field="x_t_Accion" id="x_t_Accion" name="x_t_Accion"<?php echo $af_acc_cuentas->t_Accion->EditAttributes() ?>>
<?php
if (is_array($af_acc_cuentas->t_Accion->EditValue)) {
	$arwrk = $af_acc_cuentas->t_Accion->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($af_acc_cuentas->t_Accion->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
faf_acc_cuentasadd.Lists["x_t_Accion"].Options = <?php echo (is_array($af_acc_cuentas->t_Accion->EditValue)) ? ew_ArrayToJson($af_acc_cuentas->t_Accion->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $af_acc_cuentas->t_Accion->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->c_IReseller->Visible) { // c_IReseller ?>
	<tr id="r_c_IReseller">
		<td><span id="elh_af_acc_cuentas_c_IReseller"><?php echo $af_acc_cuentas->c_IReseller->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $af_acc_cuentas->c_IReseller->CellAttributes() ?>>
<span id="el_af_acc_cuentas_c_IReseller" class="control-group">
<select class="form-control" data-field="x_c_IReseller" id="x_c_IReseller" name="x_c_IReseller"<?php echo $af_acc_cuentas->c_IReseller->EditAttributes() ?>>
<?php
if (is_array($af_acc_cuentas->c_IReseller->EditValue)) {
	$arwrk = $af_acc_cuentas->c_IReseller->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($af_acc_cuentas->c_IReseller->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
faf_acc_cuentasadd.Lists["x_c_IReseller"].Options = <?php echo (is_array($af_acc_cuentas->c_IReseller->EditValue)) ? ew_ArrayToJson($af_acc_cuentas->c_IReseller->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $af_acc_cuentas->c_IReseller->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->c_ICClass->Visible) { // c_ICClass ?>
	<tr id="r_c_ICClass">
		<td><span id="elh_af_acc_cuentas_c_ICClass"><?php echo $af_acc_cuentas->c_ICClass->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $af_acc_cuentas->c_ICClass->CellAttributes() ?>>
<span id="el_af_acc_cuentas_c_ICClass" class="control-group">
<select class="form-control" data-field="x_c_ICClass" id="x_c_ICClass" name="x_c_ICClass"<?php echo $af_acc_cuentas->c_ICClass->EditAttributes() ?>>
<?php
if (is_array($af_acc_cuentas->c_ICClass->EditValue)) {
	$arwrk = $af_acc_cuentas->c_ICClass->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($af_acc_cuentas->c_ICClass->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
faf_acc_cuentasadd.Lists["x_c_ICClass"].Options = <?php echo (is_array($af_acc_cuentas->c_ICClass->EditValue)) ? ew_ArrayToJson($af_acc_cuentas->c_ICClass->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $af_acc_cuentas->c_ICClass->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->x_DirCorreo->Visible) { // x_DirCorreo ?>
	<tr id="r_x_DirCorreo">
		<td><span id="elh_af_acc_cuentas_x_DirCorreo"><?php echo $af_acc_cuentas->x_DirCorreo->FldCaption() ?></span></td>
		<td<?php echo $af_acc_cuentas->x_DirCorreo->CellAttributes() ?>>
<span id="el_af_acc_cuentas_x_DirCorreo" class="control-group">
<input class="form-control" type="email" data-field="x_x_DirCorreo" name="x_x_DirCorreo" id="x_x_DirCorreo" size="30" maxlength="100" placeholder="example@example.com<?php //echo ew_HtmlEncode($af_acc_cuentas->x_DirCorreo->PlaceHolder) ?>" value="<?php echo $af_acc_cuentas->x_DirCorreo->EditValue ?>"<?php echo $af_acc_cuentas->x_DirCorreo->EditAttributes() ?>>
</span>
<?php echo $af_acc_cuentas->x_DirCorreo->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->x_Titulo->Visible) { // x_Titulo ?>
	<tr id="r_x_Titulo">
		<td><span id="elh_af_acc_cuentas_x_Titulo"><?php echo $af_acc_cuentas->x_Titulo->FldCaption() ?></span></td>
		<td<?php echo $af_acc_cuentas->x_Titulo->CellAttributes() ?>>
<span id="el_af_acc_cuentas_x_Titulo" class="control-group">
<input class="form-control" type="text" data-field="x_x_Titulo" name="x_x_Titulo" id="x_x_Titulo" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($af_acc_cuentas->x_Titulo->PlaceHolder) ?>" value="<?php echo $af_acc_cuentas->x_Titulo->EditValue ?>"<?php echo $af_acc_cuentas->x_Titulo->EditAttributes() ?>>
</span>
<?php echo $af_acc_cuentas->x_Titulo->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($af_acc_cuentas->x_Mensaje->Visible) { // x_Mensaje ?>
	<tr id="r_x_Mensaje">
		<td><span id="elh_af_acc_cuentas_x_Mensaje"><?php echo $af_acc_cuentas->x_Mensaje->FldCaption() ?></span></td>
		<td<?php echo $af_acc_cuentas->x_Mensaje->CellAttributes() ?>>
<span id="el_af_acc_cuentas_x_Mensaje" class="control-group">
<textarea class="form-control" data-field="x_x_Mensaje" name="x_x_Mensaje" id="x_x_Mensaje" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($af_acc_cuentas->x_Mensaje->PlaceHolder) ?>"<?php echo $af_acc_cuentas->x_Mensaje->EditAttributes() ?>><?php echo $af_acc_cuentas->x_Mensaje->EditValue ?></textarea>
</span>
<?php echo $af_acc_cuentas->x_Mensaje->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
...
<script type="text/javascript">
faf_acc_cuentasadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$af_acc_cuentas_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_acc_cuentas_add->Page_Terminate();
?>
