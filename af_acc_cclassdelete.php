...
$af_acc_cclass_delete = NULL; // Initialize page object first

class caf_acc_cclass_delete extends caf_acc_cclass {
	...
	// Page object name
	var $PageObjName = 'af_acc_cclass_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("af_acc_cclasslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in af_acc_cclass class, af_acc_cclassinfo.php
...
// Create page object
if (!isset($af_acc_cclass_delete)) $af_acc_cclass_delete = new caf_acc_cclass_delete();

// Page init
$af_acc_cclass_delete->Page_Init();

// Page main
$af_acc_cclass_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_acc_cclass_delete->Page_Render();
?>
...
// Page object
var af_acc_cclass_delete = new ew_Page("af_acc_cclass_delete");
af_acc_cclass_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = af_acc_cclass_delete.PageID; // For backward compatibility

// Form object
var faf_acc_cclassdelete = new ew_Form("faf_acc_cclassdelete");

// Form_CustomValidate event
faf_acc_cclassdelete.Form_CustomValidate = 
...
faf_acc_cclassdelete.ValidateRequired = true;
<?php } else { ?>
faf_acc_cclassdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_acc_cclassdelete.Lists["x_cl_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cclassdelete.Lists["x_t_Accion"] = {"LinkField":"x_rv_Low_Value","Ajax":null,"AutoFill":false,"DisplayFields":["x_rv_Meaning","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cclassdelete.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_acc_cclassdelete.Lists["x_c_ICClass"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($af_acc_cclass_delete->Recordset = $af_acc_cclass_delete->LoadRecordset())
	$af_acc_cclass_deleteTotalRecs = $af_acc_cclass_delete->Recordset->RecordCount(); // Get record count
if ($af_acc_cclass_deleteTotalRecs <= 0) { // No record found, exit
	if ($af_acc_cclass_delete->Recordset)
		$af_acc_cclass_delete->Recordset->Close();
	$af_acc_cclass_delete->Page_Terminate("af_acc_cclasslist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $af_acc_cclass_delete->ShowPageHeader(); ?>
<?php
$af_acc_cclass_delete->ShowMessage();
?>
<form name="faf_acc_cclassdelete" id="faf_acc_cclassdelete" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_acc_cclass">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($af_acc_cclass_delete->RecKeys as $key) { ?>
...
<?php if ($af_acc_cclass->cl_Accion->Visible) { // cl_Accion ?>
		<td><span id="elh_af_acc_cclass_cl_Accion" class="af_acc_cclass_cl_Accion"><?php echo $af_acc_cclass->cl_Accion->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_cclass->t_Accion->Visible) { // t_Accion ?>
		<td><span id="elh_af_acc_cclass_t_Accion" class="af_acc_cclass_t_Accion"><?php echo $af_acc_cclass->t_Accion->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_cclass->c_IReseller->Visible) { // c_IReseller ?>
		<td><span id="elh_af_acc_cclass_c_IReseller" class="af_acc_cclass_c_IReseller"><?php echo $af_acc_cclass->c_IReseller->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_acc_cclass->c_ICClass->Visible) { // c_ICClass ?>
		<td><span id="elh_af_acc_cclass_c_ICClass" class="af_acc_cclass_c_ICClass"><?php echo $af_acc_cclass->c_ICClass->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$af_acc_cclass_delete->RecCnt = 0;
$i = 0;
while (!$af_acc_cclass_delete->Recordset->EOF) {
	$af_acc_cclass_delete->RecCnt++;
	$af_acc_cclass_delete->RowCnt++;

	// Set row properties
	$af_acc_cclass->ResetAttrs();
	$af_acc_cclass->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$af_acc_cclass_delete->LoadRowValues($af_acc_cclass_delete->Recordset);

	// Render row
	$af_acc_cclass_delete->RenderRow();
?>
	<tr<?php echo $af_acc_cclass->RowAttributes() ?>>
<?php if ($af_acc_cclass->cl_Accion->Visible) { // cl_Accion ?>
		<td<?php echo $af_acc_cclass->cl_Accion->CellAttributes() ?>>
<span id="el<?php echo $af_acc_cclass_delete->RowCnt ?>_af_acc_cclass_cl_Accion" class="control-group af_acc_cclass_cl_Accion">
...
<span id="el<?php echo $af_acc_cclass_delete->RowCnt ?>_af_acc_cclass_t_Accion" class="control-group af_acc_cclass_t_Accion">
...
<span id="el<?php echo $af_acc_cclass_delete->RowCnt ?>_af_acc_cclass_c_IReseller" class="control-group af_acc_cclass_c_IReseller">
...
<span id="el<?php echo $af_acc_cclass_delete->RowCnt ?>_af_acc_cclass_c_ICClass" class="control-group af_acc_cclass_c_ICClass">
...
<?php
	$af_acc_cclass_delete->Recordset->MoveNext();
}
$af_acc_cclass_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
faf_acc_cclassdelete.Init();
</script>
<?php
$af_acc_cclass_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_acc_cclass_delete->Page_Terminate();
?>
