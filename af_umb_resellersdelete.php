...
$af_umb_resellers_delete = NULL; // Initialize page object first

class caf_umb_resellers_delete extends caf_umb_resellers {
	...
	// Page object name
	var $PageObjName = 'af_umb_resellers_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("af_umb_resellerslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in af_umb_resellers class, af_umb_resellersinfo.php
...
// Create page object
if (!isset($af_umb_resellers_delete)) $af_umb_resellers_delete = new caf_umb_resellers_delete();

// Page init
$af_umb_resellers_delete->Page_Init();

// Page main
$af_umb_resellers_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_umb_resellers_delete->Page_Render();
?>
...
// Page object
var af_umb_resellers_delete = new ew_Page("af_umb_resellers_delete");
af_umb_resellers_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = af_umb_resellers_delete.PageID; // For backward compatibility

// Form object
var faf_umb_resellersdelete = new ew_Form("faf_umb_resellersdelete");

// Form_CustomValidate event
faf_umb_resellersdelete.Form_CustomValidate = 
...
faf_umb_resellersdelete.ValidateRequired = true;
<?php } else { ?>
faf_umb_resellersdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_umb_resellersdelete.Lists["x_c_IDestino"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_umb_resellersdelete.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($af_umb_resellers_delete->Recordset = $af_umb_resellers_delete->LoadRecordset())
	$af_umb_resellers_deleteTotalRecs = $af_umb_resellers_delete->Recordset->RecordCount(); // Get record count
if ($af_umb_resellers_deleteTotalRecs <= 0) { // No record found, exit
	if ($af_umb_resellers_delete->Recordset)
		$af_umb_resellers_delete->Recordset->Close();
	$af_umb_resellers_delete->Page_Terminate("af_umb_resellerslist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $af_umb_resellers_delete->ShowPageHeader(); ?>
<?php
$af_umb_resellers_delete->ShowMessage();
?>
<form name="faf_umb_resellersdelete" id="faf_umb_resellersdelete" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_umb_resellers">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($af_umb_resellers_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_af_umb_resellersdelete" class="ewTable ewTableSeparate">
<?php echo $af_umb_resellers->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($af_umb_resellers->c_IDestino->Visible) { // c_IDestino ?>
		<td><span id="elh_af_umb_resellers_c_IDestino" class="af_umb_resellers_c_IDestino"><?php echo $af_umb_resellers->c_IDestino->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_umb_resellers->c_IReseller->Visible) { // c_IReseller ?>
		<td><span id="elh_af_umb_resellers_c_IReseller" class="af_umb_resellers_c_IReseller"><?php echo $af_umb_resellers->c_IReseller->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_umb_resellers->q_MinAl_Res->Visible) { // q_MinAl_Res ?>
		<td><span id="elh_af_umb_resellers_q_MinAl_Res" class="af_umb_resellers_q_MinAl_Res"><?php echo $af_umb_resellers->q_MinAl_Res->FldCaption() ?></span></td>
<?php } ?>
<?php if ($af_umb_resellers->q_MinCu_Res->Visible) { // q_MinCu_Res ?>
		<td><span id="elh_af_umb_resellers_q_MinCu_Res" class="af_umb_resellers_q_MinCu_Res"><?php echo $af_umb_resellers->q_MinCu_Res->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$af_umb_resellers_delete->RecCnt = 0;
$i = 0;
while (!$af_umb_resellers_delete->Recordset->EOF) {
	$af_umb_resellers_delete->RecCnt++;
	$af_umb_resellers_delete->RowCnt++;

	// Set row properties
	$af_umb_resellers->ResetAttrs();
	$af_umb_resellers->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$af_umb_resellers_delete->LoadRowValues($af_umb_resellers_delete->Recordset);

	// Render row
	$af_umb_resellers_delete->RenderRow();
?>
	<tr<?php echo $af_umb_resellers->RowAttributes() ?>>
<?php if ($af_umb_resellers->c_IDestino->Visible) { // c_IDestino ?>
		<td<?php echo $af_umb_resellers->c_IDestino->CellAttributes() ?>>
<span id="el<?php echo $af_umb_resellers_delete->RowCnt ?>_af_umb_resellers_c_IDestino" class="control-group af_umb_resellers_c_IDestino">
...
<span id="el<?php echo $af_umb_resellers_delete->RowCnt ?>_af_umb_resellers_c_IReseller" class="control-group af_umb_resellers_c_IReseller">
...
<span id="el<?php echo $af_umb_resellers_delete->RowCnt ?>_af_umb_resellers_q_MinAl_Res" class="control-group af_umb_resellers_q_MinAl_Res">
...
<span id="el<?php echo $af_umb_resellers_delete->RowCnt ?>_af_umb_resellers_q_MinCu_Res" class="control-group af_umb_resellers_q_MinCu_Res">
...
<?php
	$af_umb_resellers_delete->Recordset->MoveNext();
}
$af_umb_resellers_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
faf_umb_resellersdelete.Init();
</script>
<?php
$af_umb_resellers_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$af_umb_resellers_delete->Page_Terminate();
?>
