...
$af_resellers_usuario_list = NULL; // Initialize page object first

class caf_resellers_usuario_list extends caf_resellers_usuario {
	...
	// Page object name
	var $PageObjName = 'af_resellers_usuario_list';

	// Grid form hidden field names
	var $FormName = 'faf_resellers_usuariolist';
	...
	function __construct() {
		...
		$this->AddUrl = "af_resellers_usuarioadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "af_resellers_usuariodelete.php";
		$this->MultiUpdateUrl = "af_resellers_usuarioupdate.php";
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<label class=\"checkbox\"><input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->c_Usuario->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->c_IReseller->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'></label>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"btn-primary ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.faf_resellers_usuariolist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = (TRUE);
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.faf_resellers_usuariolist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_af_resellers_usuario\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_af_resellers_usuario',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.faf_resellers_usuariolist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($af_resellers_usuario_list)) $af_resellers_usuario_list = new caf_resellers_usuario_list();

// Page init
$af_resellers_usuario_list->Page_Init();

// Page main
$af_resellers_usuario_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_resellers_usuario_list->Page_Render();
?>
...
// Page object
var af_resellers_usuario_list = new ew_Page("af_resellers_usuario_list");
af_resellers_usuario_list.PageID = "list"; // Page ID
var EW_PAGE_ID = af_resellers_usuario_list.PageID; // For backward compatibility

// Form object
var faf_resellers_usuariolist = new ew_Form("faf_resellers_usuariolist");
faf_resellers_usuariolist.FormKeyCountName = '<?php echo $af_resellers_usuario_list->FormKeyCountName ?>';

// Form_CustomValidate event
faf_resellers_usuariolist.Form_CustomValidate = 
...
faf_resellers_usuariolist.ValidateRequired = true;
<?php } else { ?>
faf_resellers_usuariolist.ValidateRequired = false; 
<?php } ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$af_resellers_usuario_list->TotalRecs = $af_resellers_usuario->SelectRecordCount();
	} else {
		if ($af_resellers_usuario_list->Recordset = $af_resellers_usuario_list->LoadRecordset())
			$af_resellers_usuario_list->TotalRecs = $af_resellers_usuario_list->Recordset->RecordCount();
	}
	$af_resellers_usuario_list->StartRec = 1;
	if ($af_resellers_usuario_list->DisplayRecs <= 0 || ($af_resellers_usuario->Export <> "" && $af_resellers_usuario->ExportAll)) // Display all records
		$af_resellers_usuario_list->DisplayRecs = $af_resellers_usuario_list->TotalRecs;
	if (!($af_resellers_usuario->Export <> "" && $af_resellers_usuario->ExportAll))
		$af_resellers_usuario_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
...
<div id="filterContainer">
	...
	$(document).on('click', '#submit_filtros', function() { 
		...
		$.ajax({  
		  ...
		  success: function(html) {  
			window.location="af_resellers_usuariolist.php?start=1";
		  }
		  });
	...
	$(document).on('click','#reset_fil',function(){
		...
		$.ajax({  
		  ...
		  data: dataString,  
		  success: function(html) { 
			window.location="af_resellers_usuariolist.php";
		  }
		});
	});
...
<div id="gmp_af_resellers_usuario" class="ewGridMiddlePanel">
<?php if ($af_resellers_usuario_list->TotalRecs > 0) { ?>
<table id="tbl_af_resellers_usuariolist" class="ewTable ewTableSeparate">
...
// Render list options
$af_resellers_usuario_list->RenderListOptions();

// Render list options (header, left)
$af_resellers_usuario_list->ListOptions->Render("header", "left");
?>
<?php if ($af_resellers_usuario->c_Usuario->Visible) { // c_Usuario ?>
	<?php if ($af_resellers_usuario->SortUrl($af_resellers_usuario->c_Usuario) == "") { ?>
...
<?php if ($af_resellers_usuario->c_IReseller->Visible) { // c_IReseller ?>
	<?php if ($af_resellers_usuario->SortUrl($af_resellers_usuario->c_IReseller) == "") { ?>
...
<?php if ($af_resellers_usuario->f_Ult_Mod->Visible) { // f_Ult_Mod ?>
	<?php if ($af_resellers_usuario->SortUrl($af_resellers_usuario->f_Ult_Mod) == "") { ?>
		<td><div id="elh_af_resellers_usuario_f_Ult_Mod" class="af_resellers_usuario_f_Ult_Mod"><div class="ewTableHeaderCaption"><?php echo $af_resellers_usuario->f_Ult_Mod->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $af_resellers_usuario->SortUrl($af_resellers_usuario->f_Ult_Mod) ?>',2);"><div id="elh_af_resellers_usuario_f_Ult_Mod" class="af_resellers_usuario_f_Ult_Mod">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $af_resellers_usuario->f_Ult_Mod->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($af_resellers_usuario->f_Ult_Mod->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($af_resellers_usuario->f_Ult_Mod->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($af_resellers_usuario->c_Usuario_Ult_Mod->Visible) { // c_Usuario_Ult_Mod ?>
	<?php if ($af_resellers_usuario->SortUrl($af_resellers_usuario->c_Usuario_Ult_Mod) == "") { ?>
...
$af_resellers_usuario_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($af_resellers_usuario->ExportAll && $af_resellers_usuario->Export <> "") {
	$af_resellers_usuario_list->StopRec = $af_resellers_usuario_list->TotalRecs;
} else {

	// Set the last record to display
	if ($af_resellers_usuario_list->TotalRecs > $af_resellers_usuario_list->StartRec + $af_resellers_usuario_list->DisplayRecs - 1)
		$af_resellers_usuario_list->StopRec = $af_resellers_usuario_list->StartRec + $af_resellers_usuario_list->DisplayRecs - 1;
	else
		$af_resellers_usuario_list->StopRec = $af_resellers_usuario_list->TotalRecs;
}
$af_resellers_usuario_list->RecCnt = $af_resellers_usuario_list->StartRec - 1;
if ($af_resellers_usuario_list->Recordset && !$af_resellers_usuario_list->Recordset->EOF) {
	$af_resellers_usuario_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $af_resellers_usuario_list->StartRec > 1)
		$af_resellers_usuario_list->Recordset->Move($af_resellers_usuario_list->StartRec - 1);
} elseif (!$af_resellers_usuario->AllowAddDeleteRow && $af_resellers_usuario_list->StopRec == 0) {
	$af_resellers_usuario_list->StopRec = $af_resellers_usuario->GridAddRowCount;
}

// Initialize aggregate
$af_resellers_usuario->RowType = EW_ROWTYPE_AGGREGATEINIT;
$af_resellers_usuario->ResetAttrs();
$af_resellers_usuario_list->RenderRow();
while ($af_resellers_usuario_list->RecCnt < $af_resellers_usuario_list->StopRec) {
	$af_resellers_usuario_list->RecCnt++;
	if (intval($af_resellers_usuario_list->RecCnt) >= intval($af_resellers_usuario_list->StartRec)) {
		$af_resellers_usuario_list->RowCnt++;

		// Set up key count
		$af_resellers_usuario_list->KeyCount = $af_resellers_usuario_list->RowIndex;

		// Init row class and style
		$af_resellers_usuario->ResetAttrs();
		$af_resellers_usuario->CssClass = "";
		if ($af_resellers_usuario->CurrentAction == "gridadd") {
		} else {
			$af_resellers_usuario_list->LoadRowValues($af_resellers_usuario_list->Recordset); // Load row values
		}
		$af_resellers_usuario->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$af_resellers_usuario->RowAttrs = array_merge($af_resellers_usuario->RowAttrs, array('data-rowindex'=>$af_resellers_usuario_list->RowCnt, 'id'=>'r' . $af_resellers_usuario_list->RowCnt . '_af_resellers_usuario', 'data-rowtype'=>$af_resellers_usuario->RowType));
...
$af_resellers_usuario_list->ListOptions->Render("body", "left", $af_resellers_usuario_list->RowCnt);
?>
	<?php if ($af_resellers_usuario->c_Usuario->Visible) { // c_Usuario ?>
...
<a id="<?php echo $af_resellers_usuario_list->PageObjName . "_row_" . $af_resellers_usuario_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($af_resellers_usuario->c_IReseller->Visible) { // c_IReseller ?>
...
$af_resellers_usuario_list->ListOptions->Render("body", "right", $af_resellers_usuario_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($af_resellers_usuario->CurrentAction <> "gridadd")
		$af_resellers_usuario_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($af_resellers_usuario->CurrentAction == "") { ?>
...
// Close recordset
if ($af_resellers_usuario_list->Recordset)
	$af_resellers_usuario_list->Recordset->Close();
?>
<?php if ($af_resellers_usuario->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($af_resellers_usuario->CurrentAction <> "gridadd" && $af_resellers_usuario->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($af_resellers_usuario_list->Pager)) $af_resellers_usuario_list->Pager = new cNumericPager($af_resellers_usuario_list->StartRec, $af_resellers_usuario_list->DisplayRecs, $af_resellers_usuario_list->TotalRecs, $af_resellers_usuario_list->RecRange) ?>
<?php if ($af_resellers_usuario_list->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($af_resellers_usuario_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $af_resellers_usuario_list->PageUrl() ?>start=<?php echo $af_resellers_usuario_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($af_resellers_usuario_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $af_resellers_usuario_list->PageUrl() ?>start=<?php echo $af_resellers_usuario_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($af_resellers_usuario_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $af_resellers_usuario_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($af_resellers_usuario_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $af_resellers_usuario_list->PageUrl() ?>start=<?php echo $af_resellers_usuario_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($af_resellers_usuario_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $af_resellers_usuario_list->PageUrl() ?>start=<?php echo $af_resellers_usuario_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($af_resellers_usuario_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $af_resellers_usuario_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $af_resellers_usuario_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $af_resellers_usuario_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
...
<?php
	foreach ($af_resellers_usuario_list->OtherOptions as &$option)
...
<?php } ?>
<?php
$af_resellers_usuario_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($af_resellers_usuario->Export == "") { ?>
...
<?php
$af_resellers_usuario_list->Page_Terminate();
?>
