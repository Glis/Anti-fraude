...
$af_umb_cuentas_list = NULL; // Initialize page object first

class caf_umb_cuentas_list extends caf_umb_cuentas {
	...
	// Page object name
	var $PageObjName = 'af_umb_cuentas_list';

	// Grid form hidden field names
	var $FormName = 'faf_umb_cuentaslist';
	...
	function __construct() {
		...
		$this->AddUrl = "af_umb_cuentasadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "af_umb_cuentasdelete.php";
		$this->MultiUpdateUrl = "af_umb_cuentasupdate.php";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 4) {
			...
			$this->c_ICuenta->setFormValue($arrKeyFlds[3]);
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->q_MinAl_Cta, $bCtrl); // q_MinAl_Cta
			$this->UpdateSort($this->q_MinCu_Cta, $bCtrl); // q_MinCu_Cta
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->q_MinAl_Cta->setSort("");
				$this->q_MinCu_Cta->setSort("");
			}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<label class=\"checkbox\"><input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->c_IDestino->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->c_IReseller->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->c_ICliente->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->c_ICuenta->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'></label>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"btn-primary ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.faf_umb_cuentaslist, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = (TRUE);
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.faf_umb_cuentaslist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_af_umb_cuentas\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_af_umb_cuentas',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.faf_umb_cuentaslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($af_umb_cuentas_list)) $af_umb_cuentas_list = new caf_umb_cuentas_list();

// Page init
$af_umb_cuentas_list->Page_Init();

// Page main
$af_umb_cuentas_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$af_umb_cuentas_list->Page_Render();
?>
...
// Page object
var af_umb_cuentas_list = new ew_Page("af_umb_cuentas_list");
af_umb_cuentas_list.PageID = "list"; // Page ID
var EW_PAGE_ID = af_umb_cuentas_list.PageID; // For backward compatibility

// Form object
var faf_umb_cuentaslist = new ew_Form("faf_umb_cuentaslist");
faf_umb_cuentaslist.FormKeyCountName = '<?php echo $af_umb_cuentas_list->FormKeyCountName ?>';

// Form_CustomValidate event
faf_umb_cuentaslist.Form_CustomValidate = 
...
faf_umb_cuentaslist.ValidateRequired = true;
<?php } else { ?>
faf_umb_cuentaslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faf_umb_cuentaslist.Lists["x_c_IDestino"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_umb_cuentaslist.Lists["x_c_IReseller"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_umb_cuentaslist.Lists["x_c_ICliente"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
faf_umb_cuentaslist.Lists["x_c_ICuenta"] = {"LinkField":"x_c_Usuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_c_Usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($af_umb_cuentas_list->ExportOptions->Visible()) { ?>
<div id="page_title" class="ewListExportOptions"><?php $af_umb_cuentas_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$af_umb_cuentas_list->TotalRecs = $af_umb_cuentas->SelectRecordCount();
	} else {
		if ($af_umb_cuentas_list->Recordset = $af_umb_cuentas_list->LoadRecordset())
			$af_umb_cuentas_list->TotalRecs = $af_umb_cuentas_list->Recordset->RecordCount();
	}
	$af_umb_cuentas_list->StartRec = 1;
	if ($af_umb_cuentas_list->DisplayRecs <= 0 || ($af_umb_cuentas->Export <> "" && $af_umb_cuentas->ExportAll)) // Display all records
		$af_umb_cuentas_list->DisplayRecs = $af_umb_cuentas_list->TotalRecs;
	if (!($af_umb_cuentas->Export <> "" && $af_umb_cuentas->ExportAll))
		$af_umb_cuentas_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
...
<form name="faf_umb_cuentaslist" id="faf_umb_cuentaslist" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="af_umb_cuentas">
<div id="gmp_af_umb_cuentas" class="ewGridMiddlePanel">
<?php if ($af_umb_cuentas_list->TotalRecs > 0) { ?>
<table id="tbl_af_umb_cuentaslist" class="ewTable ewTableSeparate">
...
// Render list options
$af_umb_cuentas_list->RenderListOptions();

// Render list options (header, left)
$af_umb_cuentas_list->ListOptions->Render("header", "left");
?>
<?php if ($af_umb_cuentas->c_IDestino->Visible) { // c_IDestino ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->c_IDestino) == "") { ?>
...
<?php if ($af_umb_cuentas->c_IReseller->Visible) { // c_IReseller ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->c_IReseller) == "") { ?>
...
<?php if ($af_umb_cuentas->c_ICliente->Visible) { // c_ICliente ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->c_ICliente) == "") { ?>
...
<?php if ($af_umb_cuentas->c_ICuenta->Visible) { // c_ICuenta ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->c_ICuenta) == "") { ?>
...
<?php if ($af_umb_cuentas->q_MinAl_Cta->Visible) { // q_MinAl_Cta ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->q_MinAl_Cta) == "") { ?>
		<td><div id="elh_af_umb_cuentas_q_MinAl_Cta" class="af_umb_cuentas_q_MinAl_Cta"><div class="ewTableHeaderCaption"><?php echo $af_umb_cuentas->q_MinAl_Cta->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $af_umb_cuentas->SortUrl($af_umb_cuentas->q_MinAl_Cta) ?>',2);"><div id="elh_af_umb_cuentas_q_MinAl_Cta" class="af_umb_cuentas_q_MinAl_Cta">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $af_umb_cuentas->q_MinAl_Cta->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($af_umb_cuentas->q_MinAl_Cta->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($af_umb_cuentas->q_MinAl_Cta->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($af_umb_cuentas->q_MinCu_Cta->Visible) { // q_MinCu_Cta ?>
	<?php if ($af_umb_cuentas->SortUrl($af_umb_cuentas->q_MinCu_Cta) == "") { ?>
		<td><div id="elh_af_umb_cuentas_q_MinCu_Cta" class="af_umb_cuentas_q_MinCu_Cta"><div class="ewTableHeaderCaption"><?php echo $af_umb_cuentas->q_MinCu_Cta->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $af_umb_cuentas->SortUrl($af_umb_cuentas->q_MinCu_Cta) ?>',2);"><div id="elh_af_umb_cuentas_q_MinCu_Cta" class="af_umb_cuentas_q_MinCu_Cta">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $af_umb_cuentas->q_MinCu_Cta->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($af_umb_cuentas->q_MinCu_Cta->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($af_umb_cuentas->q_MinCu_Cta->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
$af_umb_cuentas_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($af_umb_cuentas->ExportAll && $af_umb_cuentas->Export <> "") {
	$af_umb_cuentas_list->StopRec = $af_umb_cuentas_list->TotalRecs;
} else {

	// Set the last record to display
	if ($af_umb_cuentas_list->TotalRecs > $af_umb_cuentas_list->StartRec + $af_umb_cuentas_list->DisplayRecs - 1)
		$af_umb_cuentas_list->StopRec = $af_umb_cuentas_list->StartRec + $af_umb_cuentas_list->DisplayRecs - 1;
	else
		$af_umb_cuentas_list->StopRec = $af_umb_cuentas_list->TotalRecs;
}
$af_umb_cuentas_list->RecCnt = $af_umb_cuentas_list->StartRec - 1;
if ($af_umb_cuentas_list->Recordset && !$af_umb_cuentas_list->Recordset->EOF) {
	$af_umb_cuentas_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $af_umb_cuentas_list->StartRec > 1)
		$af_umb_cuentas_list->Recordset->Move($af_umb_cuentas_list->StartRec - 1);
} elseif (!$af_umb_cuentas->AllowAddDeleteRow && $af_umb_cuentas_list->StopRec == 0) {
	$af_umb_cuentas_list->StopRec = $af_umb_cuentas->GridAddRowCount;
}

// Initialize aggregate
$af_umb_cuentas->RowType = EW_ROWTYPE_AGGREGATEINIT;
$af_umb_cuentas->ResetAttrs();
$af_umb_cuentas_list->RenderRow();
while ($af_umb_cuentas_list->RecCnt < $af_umb_cuentas_list->StopRec) {
	$af_umb_cuentas_list->RecCnt++;
	if (intval($af_umb_cuentas_list->RecCnt) >= intval($af_umb_cuentas_list->StartRec)) {
		$af_umb_cuentas_list->RowCnt++;

		// Set up key count
		$af_umb_cuentas_list->KeyCount = $af_umb_cuentas_list->RowIndex;

		// Init row class and style
		$af_umb_cuentas->ResetAttrs();
		$af_umb_cuentas->CssClass = "";
		if ($af_umb_cuentas->CurrentAction == "gridadd") {
		} else {
			$af_umb_cuentas_list->LoadRowValues($af_umb_cuentas_list->Recordset); // Load row values
		}
		$af_umb_cuentas->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$af_umb_cuentas->RowAttrs = array_merge($af_umb_cuentas->RowAttrs, array('data-rowindex'=>$af_umb_cuentas_list->RowCnt, 'id'=>'r' . $af_umb_cuentas_list->RowCnt . '_af_umb_cuentas', 'data-rowtype'=>$af_umb_cuentas->RowType));
...
$af_umb_cuentas_list->ListOptions->Render("body", "left", $af_umb_cuentas_list->RowCnt);
?>
	<?php if ($af_umb_cuentas->c_IDestino->Visible) { // c_IDestino ?>
...
<a id="<?php echo $af_umb_cuentas_list->PageObjName . "_row_" . $af_umb_cuentas_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($af_umb_cuentas->c_IReseller->Visible) { // c_IReseller ?>
...
$af_umb_cuentas_list->ListOptions->Render("body", "right", $af_umb_cuentas_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($af_umb_cuentas->CurrentAction <> "gridadd")
		$af_umb_cuentas_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($af_umb_cuentas->CurrentAction == "") { ?>
...
// Close recordset
if ($af_umb_cuentas_list->Recordset)
	$af_umb_cuentas_list->Recordset->Close();
?>
<?php if ($af_umb_cuentas->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($af_umb_cuentas->CurrentAction <> "gridadd" && $af_umb_cuentas->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($af_umb_cuentas_list->Pager)) $af_umb_cuentas_list->Pager = new cNumericPager($af_umb_cuentas_list->StartRec, $af_umb_cuentas_list->DisplayRecs, $af_umb_cuentas_list->TotalRecs, $af_umb_cuentas_list->RecRange) ?>
<?php if ($af_umb_cuentas_list->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($af_umb_cuentas_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $af_umb_cuentas_list->PageUrl() ?>start=<?php echo $af_umb_cuentas_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($af_umb_cuentas_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $af_umb_cuentas_list->PageUrl() ?>start=<?php echo $af_umb_cuentas_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($af_umb_cuentas_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $af_umb_cuentas_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($af_umb_cuentas_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $af_umb_cuentas_list->PageUrl() ?>start=<?php echo $af_umb_cuentas_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($af_umb_cuentas_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $af_umb_cuentas_list->PageUrl() ?>start=<?php echo $af_umb_cuentas_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($af_umb_cuentas_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $af_umb_cuentas_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $af_umb_cuentas_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $af_umb_cuentas_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($af_umb_cuentas_list->SearchWhere == "0=101") { ?>
...
<?php
	foreach ($af_umb_cuentas_list->OtherOptions as &$option)
...
<?php } ?>
<?php
$af_umb_cuentas_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($af_umb_cuentas->Export == "") { ?>
...
<?php
$af_umb_cuentas_list->Page_Terminate();
?>
